...
data_mov_min_max_norm_scaled <- data_mov_num %>%
  mutate_at(.vars = c(1,3,5), .funs = list(min_max_norm))

data_sho_min_max_norm_scaled <- data_sho_num %>%
  mutate_at(.vars = c(1,3,4,6), .funs = list(min_max_norm))

write.csv(data_mov_min_max_norm_scaled, file = 'data_movie_min_max_norm_scaled.csv')
write.csv(data_sho_min_max_norm_scaled, file = 'data_show_min_max_norm_scaled.csv')

# Using standardization 
z_score_norm <- function(x) {
  (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)}

data_mov_z_score_scaled<- data_mov_num %>%
  mutate_at(.vars = c(1,3,5,), .funs = list(z_score_norm))
     
data_sho_z_score_scaled<- data_sho_num %>%
  mutate_at(.vars = c(1,3,4,6), .funs = list(z_score_norm))

write.csv(data_mov_z_score_scaled, file = 'data_movie_z_score_scaled.csv')
write.csv(data_sho_z_score_scaled, file = 'data_show_z_score_scaled.csv')
...
data_mov_custom_scale <- data_mov_num %>%
  mutate_at(.vars = c(1,3,5), .funs = list(~custom_scale(., new_min = 1, new_max = 10)))

data_sho_custom_scale <- data_sho_num %>%
  mutate_at(.vars = c(1,3,4,6), .funs = list(~custom_scale(., new_min = 1, new_max = 10)))

write.csv(data_mov_custom_scale, file = 'data_movie_custom_scale.csv')
write.csv(data_sho_custom_scale, file = 'data_show_custom_scale.csv')
